 ...
 * Mendukung: radio, boolean, checkbox, matching
...
function parsePrePostYaml_(yamlStr) {
  ...
  // listMode menentukan konteks parsing saat ini
  // null | 'options' | 'answer_list' | 'pairs' | 'pair_right_options' | 'answer_map'
  let listMode = null;
  let currentPair = null;

  /** Helper: strip surrounding quotes dari string */
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (indent === 0) {
      ...
      if (colonIdx !== -1) {
        ...
        let val = stripQuotes_(trimmed.substring(colonIdx + 1).trim());
    ...
    // ── Dalam blok questions ──────────────────────────────────────────────────
    if (!inQuestions) continue;

    if (trimmed.startsWith('- ')) {
      // ---- Baris dimulai dengan "- " ----
      const content = trimmed.substring(2).trim(); // hapus "- " di awal
      const colonIdx = content.indexOf(':');
      const key = colonIdx !== -1 ? content.substring(0, colonIdx).trim() : '';
      let val = colonIdx !== -1 ? stripQuotes_(content.substring(colonIdx + 1).trim()) : stripQuotes_(content);

      // Cek apakah ini awal soal baru (indent 2, key type/name/label)
      if (indent === 2 && (key === 'type' || key === 'name' || key === 'label')) {
        // Simpan soal sebelumnya
        if (currentQuestion) result.questions.push(currentQuestion);
        currentQuestion = { type: '', name: '', label: '', options: [], pairs: [], answer: null, category: '' };
        listMode = null;
        currentPair = null;
        if (key) currentQuestion[key] = val;
        continue;
      }

      // Baris list item di dalam list mode
      if (listMode === 'options' && currentQuestion) {
        currentQuestion.options.push(stripQuotes_(trimmed.substring(2).trim()));
        continue;
      }
      if (listMode === 'answer_list' && currentQuestion) {
        if (!Array.isArray(currentQuestion.answer)) currentQuestion.answer = [];
        currentQuestion.answer.push(stripQuotes_(trimmed.substring(2).trim()));
        continue;
      }
      if ((listMode === 'pairs' || listMode === 'pair_right_options') && currentQuestion) {
        if (key === 'left') {
          currentPair = { left: val, right_options: [] };
          currentQuestion.pairs.push(currentPair);
          listMode = 'pair_details'; // mode sub-property dari pair
        } else if (currentPair && (listMode === 'pair_right_options')) {
          currentPair.right_options.push(stripQuotes_(trimmed.substring(2).trim()));
        }
        continue;
      }
      if (listMode === 'pair_details' && currentPair) {
        // Baris "- opsi" di dalam right_options
        currentPair.right_options.push(stripQuotes_(trimmed.substring(2).trim()));
        continue;
      }
    } else {
      // ---- Baris tanpa "- " (key: value atau key: block) ----
      const colonIdx = trimmed.indexOf(':');
      ...
      // ── Properti dasar soal ──
      if (key === 'type') { currentQuestion.type = val; listMode = null; }
      else if (key === 'name') { currentQuestion.name = val; listMode = null; }
      else if (key === 'label') { currentQuestion.label = val; listMode = null; }
      else if (key === 'category') { currentQuestion.category = val; listMode = null; }

      // ── options ──
      else if (key === 'options') {
        listMode = 'options';
        if (val) { // inline array
          try { currentQuestion.options = JSON.parse(val.replace(/'/g, '"')); listMode = null; } catch(e) {}
        }
      }
      ...
      // ── answer ──
        ...
        if (val.startsWith('[') && val.endsWith(']')) {
          // Inline array: answer: ["A", "B"]
          try { currentQuestion.answer = JSON.parse(val.replace(/'/g, '"')); } catch(e) { currentQuestion.answer = []; }
          listMode = null;
        } else if (val === '' || val === null) {
          // Multi-baris: akan diisi oleh baris berikutnya
          // Deteksi tipe: checkbox -> answer_list, matching -> answer_map
      ...
      // ── answer map entry (untuk matching multi-baris) ──
      else if (listMode === 'answer_map' && currentQuestion) {
        let cleanKey = stripQuotes_(key);
        if (!currentQuestion.answer || typeof currentQuestion.answer !== 'object' || Array.isArray(currentQuestion.answer)) {
          currentQuestion.answer = {};
        }
        currentQuestion.answer[cleanKey] = val;
      }
      ...
      // ── right_options (dalam pair) ──
      else if (key === 'right_options') {
        listMode = 'pair_right_options';
      }

      // ── left (dalam pair item) ──
      else if (key === 'left') {
        // Awal pair baru, tapi tanpa dash (indent lebih)
        currentPair = { left: val, right_options: [] };
        currentQuestion.pairs.push(currentPair);
